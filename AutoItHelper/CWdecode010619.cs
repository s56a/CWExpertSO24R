...
namespace CWExpert
{
    public class CWDecode
    {
       ...
       // public int activech2 = 0;
        ...
        public int bwl0 = 12;
        public int bwh0 = 36;
        public int bwl1 = 12 + FFTlen;
        public int bwh1 = 36 + FFTlen;
        ...
        public string[] scp = new string[38488];
        public string[] cona = new string[38488];
...
/*
        ...
        public void Channel()
        {
            try
            {
                ...
                for (i = 0; i < 10; i++)
                {
                    ...
                    if (i > 2)
                    {
                        for (j = bwl0 - 1; j <= bwh0 + 1; j++)
                        {
                            noise[j] += sigs[j];
                            noise[j + FFTlen] += sigs[j + FFTlen];
                        }
                    }
                }

                for (i = 0; i < F2L; i++) prag[i] = noise[i];
        ...
        public void TX1()
        {
            if (rts1 == 1) morseRunner1.SendKey(Keys.Enter);
            else if (rts1 == 4) morseRunner1.SendKey(Keys.F1);
            else if (rts1 == 5) morseRunner1.SendKey(Keys.F3);
            else if (rts1 == 3) morseRunner1.SendKey(Keys.F5);
            if (rts1 == 2 || rts1 == 3) morseRunner1.SendKey(Keys.F2);
            rts1 = 0;
        }

        public void TX2()
        {
            if (rts2 == 1) morseRunner2.SendKey(Keys.Enter);
            else if (rts2 == 4) morseRunner2.SendKey(Keys.F1);
            else if (rts2 == 5) morseRunner2.SendKey(Keys.F3);
            else if (rts2 == 3) morseRunner2.SendKey(Keys.F5);
            if (rts2 == 2 || rts2 == 3) morseRunner2.SendKey(Keys.F2);
            rts2 = 0;
        }
        ...
        private void SCPLoad()
        {
            try
            {
                ...
                System.IO.StreamReader file1 = new System.IO.StreamReader("mr_db.txt");
            ...
            try
            {
                ...
                for (int z = bwl1; z <= bwh1; z++)
                {
                    ...
                    if (output[z].Contains(" ")) // && enable[z] && maxi1[z] > maxi1[z - 1] && maxi1[z] > maxi1[z + 1])  // peak!)
                    {
                        ...
                        {
                            if (freq2 > FFTlen && z <= freq2 + 1 && mystr.Contains("N"))
                            {
                                int i = mystr.Length;
                ...
                for (int z = bwl0; z <= bwh0; z++)
                {
                    ...
                    if (output[z].Contains(" ")) // && enable[z] && maxi1[z] > maxi1[z - 1] && maxi1[z] > maxi1[z + 1])  // peak!)
                    {
                        ...
                        else if (mystr.Length >= 3)
                        {
                            ...
                            if (freq1 > 0 && z <= freq1 + 1) // && mystr.Contains("N"))
                            {
